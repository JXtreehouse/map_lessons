...
var _transformFeedback = require('./transform-feedback');
...
var Program = /*#__PURE__*/function (_Resource) {
  ...
  function Program(gl) {
    ...
    _this.vertexAttributes = _vertexArray2.default.getDefaultArray(gl);
    Object.seal(_this);
  ...
  _createClass(Program, [{
    ...
    value: function initialize() {
      var _ref = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {},
          ...
          bufferMode = _ref$bufferMode === undefined ? _api2.default.SEPARATE_ATTRIBS : _ref$bufferMode;
      ...
      if (varyings) {
        (0, _context.assertWebGL2Context)(this.gl);
    ...
    value: function draw(_ref2) {
      ...
      var _ref2$drawMode = _ref2.drawMode,
          drawMode = _ref2$drawMode === undefined ? _api2.default.TRIANGLES : _ref2$drawMode,
          ...
          indexType = _ref2$indexType === undefined ? _api2.default.UNSIGNED_SHORT : _ref2$indexType,
      ...
      vertexArray = vertexArray || _vertexArray2.default.getDefaultArray(this.gl);
      vertexArray.bind(function () {

        _this2.gl.useProgram(_this2.handle);

        if (transformFeedback) {
          if (parameters[_api2.default.RASTERIZER_DISCARD]) {
            // bypass fragment shader
            _this2.gl.enable(_api2.default.RASTERIZER_DISCARD);
          }

          var primitiveMode = (0, _transformFeedback.getTransformFeedbackMode)({ drawMode: drawMode });
        ...
        } else if (isIndexed && (0, _context.isWebGL2)(_this2.gl) && !isNaN(start) && !isNaN(end)) {
        ...
        if (transformFeedback) {
          ...
          if (parameters[_api2.default.RASTERIZER_DISCARD]) {
            ...
            _this2.gl.disable(_api2.default.RASTERIZER_DISCARD);
          }
        }
      });
    ...
    value: function setBuffers(buffers) {
      ...
      for (var location = 0; location < locations.length; ++location) {
        ...
        } else if (buffer instanceof _buffer3.default) {
    ...
    value: function unsetBuffers() {
      ...
      this.gl.bindBuffer(_api2.default.ELEMENT_ARRAY_BUFFER, null);
      return this;
    }
    ...
    value: function setUniforms(uniforms) {
      ...
      for (var uniformName in uniforms) {
        ...
        if (uniformSetter) {
          if (uniform instanceof _framebuffer2.default) {
    ...
    value: function getAttributeCount() {
      return this._getParameter(_api2.default.ACTIVE_ATTRIBUTES);
    }
    ...
    value: function getUniformCount() {
      return this._getParameter(_api2.default.ACTIVE_UNIFORMS);
    }
    ...
    value: function _compileAndLink() {
      ...
      _utils.log.time(LOG_PROGRAM_PERF_PRIORITY, 'linkProgram for ' + this._getName());
      gl.linkProgram(this.handle);
      _utils.log.timeEnd(LOG_PROGRAM_PERF_PRIORITY, 'linkProgram for ' + this._getName());
    ...
    value: function _checkBuffers() {
      for (var attributeName in this._attributeLocations) {
        if (!this._filledLocations[attributeName] && !this._warnedLocations[attributeName]) {
          ...
          _utils.log.warn('Program ' + this.id + ': Attribute ' + location + ':' + attributeName + ' not supplied');
    ...
    value: function _sortBuffersByLocation(buffers) {
      ...
      for (var bufferName in buffers) {
        ...
        if (location === undefined) {
          if (buffer.target === _api2.default.ELEMENT_ARRAY_BUFFER && elements) {
            throw new Error(this._print(bufferName) + ' duplicate GL.ELEMENT_ARRAY_BUFFER');
          } else if (buffer.target === _api2.default.ELEMENT_ARRAY_BUFFER) {
            elements = bufferName;
          } else if (!this._warnedLocations[bufferName]) {
            _utils.log.log(2, this._print(bufferName) + ' not used');
            this._warnedLocations[bufferName] = true;
          }
        } else {
          if (buffer.target === _api2.default.ELEMENT_ARRAY_BUFFER) {
    ...
    value: function _getOptionsFromHandle(handle) {
      ...
      try {
        for (var _iterator = shaderHandles[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          ...
          var type = this.gl.getShaderParameter(this.handle, _api2.default.SHADER_TYPE);
          switch (type) {
            case _api2.default.VERTEX_SHADER:
              opts.vs = new _shader.VertexShader({ handle: shaderHandle });
              break;
            case _api2.default.FRAGMENT_SHADER:
              opts.fs = new _shader.FragmentShader({ handle: shaderHandle });
              break;
            default:
          }
        }
...
function getVaryingMap(varyings, bufferMode) {
  ...
  try {
    for (var _iterator2 = varyings[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
      ...
      if (bufferMode === _api2.default.SEPARATE_ATTRIBS) {
